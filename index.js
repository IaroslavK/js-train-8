 ...
 */
// const arr = Array.from(obj, function (elem, index) {
//     return `${this.prefix}${elem}`;
// });
function getUsersFromDatabase(array) {
    return Array.from(array, function (elem) {
        return {
            id: elem.id,
            firstName: elem.name.toUpperCase(),
            years: elem.age,
        };
    });
...
function findElementIndexes(arr, element) {
    if (Array.isArray(arr)) {
        let array = Array.of(arr.indexOf(element), arr.lastIndexOf(element));
        if (
...
console.log("Завдання:8 =============================="); //([1, 2, 3, 4, 5, 2], 2)
...
function customIncludes(arr, element) {
    if (Array.isArray(arr)) {
        if (arr.includes(element)) {
            let result = arr.filter((el) => el === element).length;
            console.log(result);
